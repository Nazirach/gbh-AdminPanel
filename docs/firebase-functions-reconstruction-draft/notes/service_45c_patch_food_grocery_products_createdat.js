...
async function run() {
  const lines = [];
  lines.push("# SERVICE 45C PATCH FOOD GROCERY PRODUCTS CREATEDAT");
  lines.push("");
  ...
  if (!fs.existsSync(serviceAccountPath)) {
    lines.push("STATUS: SERVICE_45C_SERVICE_ACCOUNT_NOT_FOUND");
  ...
  for (const vendorDoc of vendorSnap.docs) {
    ...
    for (const productDoc of productSnap.docs) {
      ...
      await productDoc.ref.update({
        ...
        step45cCreatedAtPatch: true,
        step45cCreatedAtPatchAt: Timestamp.now(),
      });
  ...
  if (verifyPublishTotal > 0 && verifyOrderByTotal === verifyPublishTotal) {
    lines.push("STATUS: SERVICE_45C_FOOD_GROCERY_CREATEDAT_PATCH_PASS");
  } else if (patchedProducts > 0 && verifyOrderByTotal > 0) {
    lines.push("STATUS: SERVICE_45C_FOOD_GROCERY_CREATEDAT_PATCH_PARTIAL");
  } else {
    lines.push("STATUS: SERVICE_45C_FOOD_GROCERY_CREATEDAT_PATCH_RISK");
  }
...
run().catch((e) => {
  fs.writeFileSync(outPath, "STATUS: SERVICE_45C_ERROR\r\n" + e.stack, "utf8");
  console.error(e);
  process.exitCode = 1;
});