...
const SECTION_ID = "custom_akap_ticket";
const CATEGORY_ID = "custom_akap_ticket_category";

const akapVendors = [
  {
    id: "custom_akap_vendor_lhokseumawe_medan",
    name: "AKAP Lhokseumawe - Medan",
    description: "Pemesanan tiket bus AKAP rute Lhokseumawe, Aceh, Medan, dan sekitarnya."
  },
  {
    id: "custom_akap_vendor_aceh_jakarta",
    name: "AKAP Aceh - Jakarta",
    description: "Layanan tiket perjalanan jarak jauh Aceh, Sumatera, Jawa, dan Jakarta."
  },
  {
    id: "custom_akap_vendor_sumatra_express",
    name: "Sumatra Express Ticket",
    description: "Agen tiket bus lintas kota dan lintas provinsi wilayah Sumatera."
  }
];
...
async function main() {
  ...
  const vendorTemplate =
    await getTemplate("vendors", "publish", true) ||
  ...
  const md = [];
  md.push("# ADD AKAP TICKET SERVICE DATA");
  md.push("");
  md.push(`Generated: ${new Date().toISOString()}`);
  md.push("");
  md.push("Mode: controlled Firebase write. Add/update AKAP Ticket category/vendors only. No delete.");
  md.push("");
  ...
  md.push("STATUS: AKAP_TICKET_SERVICE_DATA_ADDED");

  fs.writeFileSync(outPath, md.join("\n"), "utf8");

  console.log("STATUS: AKAP_TICKET_SERVICE_DATA_ADDED");
...
main().catch(e => {
  console.log("STATUS: AKAP_TICKET_SERVICE_DATA_ADD_ERROR");
