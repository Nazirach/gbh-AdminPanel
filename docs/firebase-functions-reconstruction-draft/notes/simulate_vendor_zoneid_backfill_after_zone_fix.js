...
(async () => {
  const zonesSnap = await db.collection('zone').where('publish', '==', true).get();
  ...
  lines.push('# FIRESTORE VENDOR ZONEID BACKFILL SIMULATION AFTER ZONE FIX');
  lines.push('');
  ...
  lines.push('NO_ZONE_MATCH: ' + noZoneMatch);
  ...
  lines.push('STATUS: FIRESTORE_VENDOR_ZONEID_BACKFILL_SIMULATION_AFTER_ZONE_FIX_CREATED');

  fs.writeFileSync(process.argv[2], lines.join('\r\n'), 'utf8');
  console.log(lines.join('\n'));
})().catch((err) => {
  console.error('SIMULATION_AFTER_ZONE_FIX_FAILED:', err);
  process.exit(1);
});
